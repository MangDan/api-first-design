...
**변경 안함** API Blueprint 버전 정도로 생각하면 됩니다. 현재 API Blueprint spec은 1A revision 9 입니다.
```markdown
FORMAT: 1A
```

**변경 안함** 실제 서비스할 Production 서버의 주소입니다. 처음에는 Production 환경이 없기 때문에 변경하지 않습니다.
```markdown
HOST: http://polls.apiblueprint.org/
```

**변경 안함** API 문서 이름입니다. 한글도 가능합니다.
```markdown
# Movie API
```

**변경** API에 대한 설명, 소개를 적는 부분입니다. 아래와 같이 수정합니다.
```markdown
영화 정보를 제공하는 API 입니다.
```
